...
namespace Unity.RenderStreaming
{
    ...
    {
        private static readonly Dictionary<CustomRemoteInput, uint> s_mapRemoteInputAndInputUserId;
        private static readonly List<CustomRemoteInput> s_listRemoteInput;

        static RemoteInputReceiver()
        {
            s_mapRemoteInputAndInputUserId = new Dictionary<CustomRemoteInput, uint>();
            s_listRemoteInput = new List<CustomRemoteInput>();
        }
        ...
        public static IReadOnlyList<CustomRemoteInput> All()
        {
            return s_listRemoteInput;
        }

        public static CustomRemoteInput Create()
        {
            ...
            CustomRemoteInput remoteInput = new CustomRemoteInput(ref user);
        ...
        internal static void Delete(CustomRemoteInput remoteInput)
        {
            if (remoteInput == null)
            {
    ...
    public class CustomRemoteInput : IInput, IDisposable
    {
        ...
        internal CustomRemoteInput(ref InputUser user)
        {
            ...
            RemoteGyroscope = user.pairedDevices.FirstOrDefault(device => device is Gyroscope) as Gyroscope;
        }
        ...
        public void ProcessInput(byte[] bytes)
        {
            ...
            switch ((EventType)bytes[0])
            {
                ...
                case EventType.Gyroscope:
                    Debug.Log("bytes:" + bytes[0] + " , beta: " + bytes[2] + " , gamma: " + bytes[3]);
                    //float alpha = ((float) BitConverter.ToInt32(bytes, 4)) / 1000;
                    //float beta = ((float) BitConverter.ToInt32(bytes, 8)) / 1000;
                    //float gamma = ((float) BitConverter.ToInt32(bytes, 12)) / 1000;
                    var alpha = BitConverter.ToDouble(bytes, 1);
                    var beta = BitConverter.ToDouble(bytes, 9);
                    var gamma = BitConverter.ToDouble(bytes, 17);
                    //InputSystem.QueueStateEvent(RemoteGyroscope, m_gyroscopeState);
                    Quaternion camRotation = Quaternion.Euler((float)beta, (float)gamma, (float)alpha);
